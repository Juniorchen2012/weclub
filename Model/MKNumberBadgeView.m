...
{	
	self.opaque = NO;
	self.pad = 2;
	self.font = [UIFont boldSystemFontOfSize:10.5];
	self.shadow = YES;
...
- (void)drawRect:(CGRect)rect 
{
	...
	NSString* numberString = [NSString stringWithFormat:@"%@",self.valueStr];
	
	
	CGSize numberSize = [numberString sizeWithFont:self.font];
		
	CGPathRef badgePath = [self newBadgePathForTextSize:[numberString sizeWithFont:[UIFont systemFontOfSize:9]]];
