...
-(void)setupView
{
    ...
    NSLog(@"setupView : x: %f, y: %f,h: %f,w: %f",
    ...
    if (_contentView.frame.origin.x != 0) {
        ...
        [_contentView setFrame:CGRectMake(_contentView.frame.origin.x, 62, _contentView.frame.size.width, _contentView.frame.size.height)];
    }
    [_contentView setNeedsDisplay];
...
//    _contentView.relativeOrigin = fromPoint;
...
-(CGPoint)originFromView:(UIView*)fromView
{
    ...
    if([_contentView arrowDirection] == FPPopoverArrowDirectionUp)
    {
        ...
        p.y = fromView.frame.origin.y + fromView.frame.size.height - 4;
    }
    ...
    else if([_contentView arrowDirection] == FPPopoverArrowDirectionRight)
    {
        ...
        p.x = fromView.frame.origin.x + fromView.frame.size.width/2.0;
        p.y = fromView.frame.origin.y + fromView.frame.size.height + 15;
    }
    return p;
}
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
//    [v setFrame:CGRectMake(v.frame.origin.x, v.frame.origin.y, v.frame.size.width, v.frame.size.height - 4)];
    ...
    CGFloat hb = [self parentHeight] -  (p.y + v.frame.size.height - 4); //on the bottom
       ...
       (self.arrowDirection == FPPopoverArrowDirectionAny && best_h >= best_w))
    {
        ...
        else
        {
            ...
            r.origin.x = p.x + v.frame.size.width/2.0 - r.size.width/2.0;
            r.origin.y = p.y + v.frame.size.height - 4;
        }
    }
    ...
    else 
    {
        ...
        if(wl == best_w || self.arrowDirection == FPPopoverArrowDirectionRight)
        {
            ...
            r.origin.y = p.y + (v.frame.size.height - 4)/2.0 - r.size.height/2.0;
