...
//        }
        ...
        faceView.contentSize = CGSizeMake(((FACE_BUTTON_NUM/28) + 1)*320, 190);
        ...
        for (int i = 1; i<=(28*((FACE_BUTTON_NUM/28)+1)); i++) {
            ...
            if (i%28 == 0) {
                ...
                back.frame = CGRectMake((((i-1)%28)%7)*44+6+((i-1)/28*320), (((i-1)%28)/7)*44+15, 38, 27);
                [faceView addSubview:back];
                backButtonNum ++;
                continue;
            }
            if (i > (FACE_BUTTON_NUM + FACE_BUTTON_NUM/28)) {
                continue;
            }
            //计算每一个表情按钮的坐标和在哪一屏
            faceButton.frame = CGRectMake((((i-1)%28)%7)*44+6+((i-1)/28*320), (((i-1)%28)/7)*44+8, 44, 44);
            
            [faceButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%03d",i - backButtonNum]] forState:UIControlStateNormal];
        ...
        facePageControl.numberOfPages = FACE_BUTTON_NUM/28 + 1;
        ...
        [sendButton setImage:[UIImage imageNamed:@"button_big_grey"] forState:UIControlStateNormal];
        [sendButton setImage:[UIImage imageNamed:@"backFaceSelect"] forState:UIControlStateSelected];
        [sendButton addTarget:self action:@selector(sendFaceText) forControlEvents:UIControlEventTouchUpInside];
        sendButton.frame = CGRectMake(268, 185, 40, 27);
        sendLabel = [[UILabel alloc] initWithFrame:CGRectMake(270, 186, 35, 25)];
        sendLabel.text = @"发送";
...
- (id)initWithIsShowSendButton:(BOOL)isShowSendButton{
    self = [self init];
    if (self) {
        [sendButton setHidden:!isShowSendButton];
        [sendLabel setHidden:!isShowSendButton];
        return self;
    }
    return nil;
}
...
- (void)faceButton:(id)sender {
    ...
    if (self.inputTextField) {
        ...
        [faceString appendString:[_faceMap objectForKey:[NSString stringWithFormat:@"%03d",i - (int)(i/28)]]];
    ...
    if (self.inputTextView) {
        ...
        if (-1 == count) {
            ...
            self.inputTextView.text = [NSString stringWithFormat:@"%@%@%@",[self.inputTextView.text substringToIndex:[self.inputTextView.text length]-count],[_faceMap objectForKey:[NSString stringWithFormat:@"%03d",i - (int)(i/28)]],[self.inputTextView.text substringFromIndex:[self.inputTextView.text length]-count]];
        }
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"textViewChange" object:nil];
        [faceString release];
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:@"FACEBOARD_NOTIFICATION" object:nil];
