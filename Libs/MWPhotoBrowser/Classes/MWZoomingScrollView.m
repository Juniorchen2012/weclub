...
- (id)initWithPhotoBrowser:(MWPhotoBrowser *)browser zommingDelegate:(id <MWZoomingScrollDelegate>) zoomDelegate{
    if ((self = [super init])) {
        ...
        self.zoomingDelegate = zoomDelegate;
        ...
        tintLabel = [[UILabel alloc]initWithFrame:CGRectMake(110, 150, 100, 30)];
        tintLabel.text = @"WeClub";
        tintLabel.backgroundColor = [UIColor clearColor];
        tintLabel.textColor = [UIColor whiteColor];
        tintLabel.font  = [UIFont fontWithName:@"Times New Roman" size:30];
...
- (void)displayImage {
	if (_photo && _photoImageView.image == nil) {
		...
		if (img) {
            ...
            tintLabel.hidden = YES;
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CHANGE_RIGHTBARBUTTON_ITEM_STATE" object:[NSNumber numberWithBool:YES]];
            ...
            tintLabel.hidden = NO;
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CHANGE_RIGHTBARBUTTON_ITEM_STATE" object:[NSNumber numberWithBool:NO]];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    [zoomingDelegate ScrollViewDidZoomin];
}
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(float)scale{
    [_photoBrowser cancelControlHiding];
//    NSLog(@"\nv.height : %f\ns.height : %f\nv.width : %f\ns.width : %f",view.frame.size.height, scrollView.frame.size.height, view.frame.size.width, scrollView.frame.size.width);
    if (view.frame.size.width < scrollView.frame.size.width + 20) {
        [zoomingDelegate ScrollViewDidZoomout];
    }
}
