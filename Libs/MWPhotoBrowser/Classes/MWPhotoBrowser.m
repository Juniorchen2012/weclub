...
- (id)init {
    if ((self = [super init])) {
        ...
        //added by chao
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(changeStateOfrightBarButtonItem:)
                                                     name:@"CHANGE_RIGHTBARBUTTON_ITEM_STATE"
                                                   object:nil];
    }
    return self;
}
...
- (void)didReceiveMemoryWarning {
    ...
    NSLog(@"MWPhotoBrowser MemoryWarning");
    [self releaseAllUnderlyingPhotos];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        doneButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"保存", nil) style:UIBarButtonItemStylePlain target:self action:@selector(savePhoto)] autorelease];
        // Set appearance
        ...
        UIBarButtonItem *saveButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"关闭", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)] autorelease];
        // Set appearance
        if ([UIBarButtonItem respondsToSelector:@selector(appearance)]) {
            [doneButton setBackgroundImage:nil forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
            [doneButton setBackgroundImage:nil forState:UIControlStateNormal barMetrics:UIBarMetricsLandscapePhone];
...
//added by chao
-(void)changeStateOfrightBarButtonItem:(NSNotification *)notification{
...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
			...
			if (!page) {
				page = [[[MWZoomingScrollView alloc] initWithPhotoBrowser:self zommingDelegate:self] autorelease];
			}
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
		self.navigationItem.title = [NSString stringWithFormat:@"%i %@ %i", _currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), [self numberOfPhotos]];
	} else {
...
//            }
            ...
            if ([MFMailComposeViewController canSendMail]) {
                ...
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"取消", nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:NSLocalizedString(@"保存", nil),  nil] autorelease];
            }
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"Failed", @"Informing the user a process has failed") : NSLocalizedString(@"完成", @"Informing the user an item has been saved")];
    [self hideControlsAfterDelay]; // Continue as normal...
}
...
#pragma mark MWZoomingScroll Delegate
